/* Aztec C II Vers. 1.05h 6502  (C) 1982, 1983 by Manx Software Systems
   MS-DOS (Windows XP) Cross-Compiler
   WinVICE Version 1.22
   41 DHRYSTONES/SEC */
 ...
 *
 */
/* Bill Buckels 2013 (C64 TOD time function) */
#define SETCLOCK 56335
#define THOUR 56331
#define TMIN  56330
#define TSEC  56329
#define TENTH 56328
/* macros for converting to and from C64 BCD format */
#define atobcd(x) (((x/10)<<4)|(x-((x/10)*10)))
#define bcdtoa(x) (((x>>4)*10) + (x&0xf))
#define CMAIN 1
#if 0
#include <poke.h>
#include <colors.h>
...
#define C64_BWHITE 1
#define scr_clear clrscr
#define scr_txtcolor textcolor
#define scr_reset()
#define kbflush()
#define getch cgetc
#endif
/* just initializing timer to zeros */
initTOD()
{
    poke[SETCLOCK] &= 127; /* UNSET TIME OF DAY CLOCK - CRB */
	/* A specific sequence of events must be followed
	   for proper setting and reading of TOD */
	poke[THOUR] = 0;
	poke[TMIN] =  0;
	poke[TSEC] =  0;
	poke[TENTH] = 0;
}
/* time function using the  TOD */
long	time(p)
long	*p;
{
		/* Bill Buckels 2013 - don't use around midnight
		for DHRY calculation */
	long t;
	int h,m,s,tn,hr,min,sec;
	h = peek[THOUR];
	m = peek[TMIN];
	s = peek[TSEC];
	tn = peek[TENTH];
...
/* Bill Buckels 2013 */
#define	NOENUM 1		/* Define if compiler has no enum's */
...
/* Bill Buckels 2013 (in realtime.h) */
#define TIME 1			/* Use time(2) time function */
...
main()
{
    ...
    scr_txtcolor(C64_BWHITE);   /* bwhite text */
   	printf("DHRYSTONE 1.1 REINHOLD P. WEICKER 1984\n");
   	printf("C VERSION RICK RICHARDSON 1984,1986\n");
    printf("AZTEC C65 C64 BILL BUCKELS 2013\n");
    /* Bill Buckels 2013 - install the NSC driver */
	initTOD();
	Proc0();
    /* Bill Buckels 2013 */
	prtime("STOP:");
